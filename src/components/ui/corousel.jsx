...
const Slide = ({
  ...
  const { image, ticketLink, title } = slide;
      ...
      <li
        ...
        <article
          ...
          <div className="flex justify-center">
            <a href={ticketLink} target="blank" rel="noopener noreferrer"
...
export default function Carousel({
  ...
  const sortedArr = slides.slice(0,4)

  const handlePreviousClick = () => {
    const previous = current - 1;
    setCurrent(previous < 0 ? sortedArr.length - 1 : previous);
  };

  const handleNextClick = () => {
    const next = current + 1;
    setCurrent(next === sortedArr.length ? 0 : next);
  };
   ...
   <div>
    ...
    (<div className="text-center text-gray-400 h-[400px]">Loading movies...</div>):
     ...
     <ul
       ...
       style={{
         transform: `translateX(-${current * (100 / sortedArr.length)}%)`,
       }}>
       {sortedArr.map((slide, index) => (
         <Slide
           key={index}
           slide={slide}
           index={index}
